...
(function () {
  ...
  async function loadImageByNumber(folder, number) {
    for (const ext of IMAGE_EXTENSIONS) {
      const path = `images/${folder}/${number}.${ext}`;
      if (await imageExists(path)) return path;
    }
    return '';
  }

  async function loadImagesFromFolder(folder, maxAttempts = 30) {
    const images = [];

    for (let current = 1; current <= maxAttempts; current++) {
      const path = await loadImageByNumber(folder, current);
  ...
  async function init() {
    ...
    const c = CONFIG;
    const sections = getSectionsConfig(c);
    ...
    if (sections.story) buildStoryText(c);
    buildLocation(c);
    buildAccount(c);
    ...
    const [storyImages, galleryImages] = await Promise.all([
      ...
      sections.gallery ? loadImagesFromFolder('gallery', maxImages) : Promise.resolve([])
    ]);

    // Render image-dependent sections
    if (sections.story) buildStoryImages(storyImages);
    if (sections.gallery) buildGallery(galleryImages);

    // Remove loading state
    hideLoadingState(sections);
  ...
  // ── Optional Sections ──
  function getSectionsConfig(c) {
    return {
      story: c.sections?.story !== false,
      gallery: c.sections?.gallery !== false
    };
  }

  function applySectionVisibility(sections) {
    if (!sections.story) $('.story')?.remove();
    if (!sections.gallery) $('.gallery')?.remove();
  }

  // ── Loading State ──
  function showLoadingState(sections) {
    const storyImagesEl = $('.story-images');
    const galleryGrid = $('.gallery-grid');
    if (sections.story && storyImagesEl) storyImagesEl.classList.add('loading');
    if (sections.gallery && galleryGrid) galleryGrid.classList.add('loading');
  }

  function hideLoadingState(sections) {
    const storyImagesEl = $('.story-images');
    const galleryGrid = $('.gallery-grid');
    if (sections.story && storyImagesEl) storyImagesEl.classList.remove('loading');
    if (sections.gallery && galleryGrid) galleryGrid.classList.remove('loading');
  }

  // ── Hero ──
  async function buildHero(c, dateInfo, timeText) {
    const heroImg = $('.hero-image');
    if (heroImg) {
      heroImg.alt = `${c.groom.name} & ${c.bride.name}`;
      const heroSrc = await loadImageByNumber('hero', 1);
  ...
  // ── Location ──
  async function buildLocation(c) {
    ...
    if (mapImg) {
      ...
      const mapSrc = await loadImageByNumber('location', 1);
